...
const normalizeInstance = (instance = {}, fallbackProfile) => ({
  ...(() => {
    const license = normalizeLicense(instance.license, instance.plan);
...
const normalizeData = (nextData) => ({
  ...(() => {
    const fallbackProfile = nextData.businessProfile
      || nextData.users?.find(user => ["owner", "admin"].includes(user.role))?.businessProfile
      || nextData.users?.[0]?.businessProfile;
    ...
    return {
      ...
      users: (nextData.users || []).map(({ businessProfile, ...user }) => ({
        ...user,
        role: normalizeRole(user.role),
        instanceId: user.instanceId || primaryInstanceId,
      })),
